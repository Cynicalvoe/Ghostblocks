...
import com.hadenwatne.ghostblocks.files.Configuration;
import com.hadenwatne.ghostblocks.files.Messages;
import com.hadenwatne.ghostblocks.files.Tempfile;
...
public class GhostBlocks extends JavaPlugin{
	...
	public ItemStack getGhostblockItem(Material type, int amt, int data){
		ItemStack item = new ItemStack(type, amt);
		((Damageable)item.getItemMeta()).setDamage(data);
		ItemMeta im = item.getItemMeta();
		...
		for(int i=0; i<lore.size(); i++) {
			lore.set(i, lore.get(i).replaceAll("%TYPE%", type.toString()).replaceAll("%DATA%", Integer.toString(data)));
		}
